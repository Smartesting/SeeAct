...
benchmark_testing_sys_prompt = '''
Imagine that you are imitating a manual tester performing a test case on a website. You receive a test case in natural language and you should follow it and determine its verdict. Along the way, you must make sure that, for each test step, the assertions are verified before moving to the next step. A test step might involve multiple actions. At each stage, you can see the webpage like humans by a screenshot and know the previous actions before the current action decided by yourself through recorded history. You need to decide on the first following action to take. You can click on an element with the mouse, select an option, type text or press Enter with the keyboard. (For your understanding, they are like the click(), select_option() type() and keyboard.press('Enter') functions in playwright respectively) One next step means one operation within the four. Unlike humans, for typing (e.g., in text areas, text boxes) and selecting (e.g., from dropdown menus or <select> elements), you should try directly typing the input or selecting the choice, bypassing the need for an initial click. You should not attempt to create accounts, log in or do the final submission. Terminate when you deem the task complete or if it requires potentially harmful actions.'''

benchmark_testing_question_description_new_exp4 = '''The screenshot below shows the webpage you see. Follow the following guidance to think step by step before outlining the next action step at the current stage:
...
(Test Case Progress)
Based on the past actions, determine the state of the test case. Go through each step, one by one, and give it a status: {DONE, CURRENT, TODO}. Finish by confirming the current test step. 

(Screenshot Details Analysis)
Closely examine the screenshot to check the status of every part of the webpage to understand what you can operate with and what has been set or completed. You should closely examine the screenshot details to see what steps have been completed by previous actions even though you are given the textual previous actions. Because the textual history may not clearly and sufficiently record some effects of previous actions, you should closely evaluate the status of every part of the webpage to understand what you have done.

(Test Step Assertion Control)
Based on past actions, if any, control the test step assertion. Make a numbered list of all the atomic assertions and, for each one, based on what you see in the screenshot, assign a status {VERIFIED, NOT VERIFIED}. If all assertions are verifieds, write the sentence "/!\\ All assertions have been verified /!\\"
...
benchmark_testing_action_format = "ACTION: Choose an action from {CLICK, SELECT, TYPE, PRESS ENTER, TERMINATE, NONE}."

benchmark_testing_value_format = "VALUE: Provide additional input based on ACTION.\n\nThe VALUE means:\nIf ACTION == TYPE, specify the " \
               "text to be typed.\nIf ACTION == SELECT, indicate the option to be chosen. Revise the selection value to align with the available options within the element.\nIf ACTION == CLICK, PRESS ENTER, TERMINATE or NONE, " \
               "write \"None\"."

benchmark_testing_choice_prompt_dict = {
    "system_prompt": benchmark_testing_sys_prompt,

    "question_description": benchmark_testing_question_description_new_exp4,
...
ELEMENT: The uppercase letter of your choice. (No need for PRESS ENTER)""",
    ...
    "action_format": f"{benchmark_testing_action_format}",

    "value_format": f"{benchmark_testing_value_format}"
}
    ...
    if experiment_split in ["text","text_choice","4api"]:
        ...
        prompt_list.append(
            generate_new_referring_prompt(referring_description=referring_input, element_format=element_format_input,
                                          ...
                                          choices=choices))
        return prompt_list
    elif experiment_split in ["benckmark_testing", "benchmark"]:
        system_prompt_input = benchmark_testing_choice_prompt_dict["system_prompt"]
        question_description_input = benchmark_testing_choice_prompt_dict["question_description"]
        referring_input = benchmark_testing_choice_prompt_dict["referring_description"]
        element_format_input = benchmark_testing_choice_prompt_dict["element_format"]
        action_format_input = benchmark_testing_choice_prompt_dict["action_format"]
        value_format_input = benchmark_testing_choice_prompt_dict["value_format"]
        prompt_list = []
